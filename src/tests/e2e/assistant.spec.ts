...
test("avatar aparece sem interromper a navegação (Cenário 1)", async ({
  page,
}) => {
  await page.goto("/");
  await expect(
    page.getByRole("button", { name: "Abrir Assistente Virtual WJB" }),
  ).toBeVisible();
  // A navegação normal do site continua funcionando com o bot ignorado.
  await expect(page.getByRole("heading", { level: 1 })).toBeVisible();
});

test("selecionar um serviço mostra conteúdo real + CTAs (Cenário 2)", async ({
  page,
}) => {
  await page.goto("/");
  await page
...
test("selecionar o plano MEI navega para a rota real (Cenário 3)", async ({
  page,
}) => {
  await page.goto("/");
  await page
...
test("selecionar Dúvidas frequentes navega para a página real (Cenário 9)", async ({
  page,
}) => {
  await page.goto("/");
  await page
