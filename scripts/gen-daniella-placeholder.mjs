/**
 * Usuário pediu (2026-08-31) pra voltar o retrato da Daniella Santana pra
 * placeholder, igual ao do Diego, em vez da foto real já aplicada. Gera o
 * placeholder num arquivo separado (`daniella-santana-portrait-placeholder.webp`)
 * — a foto real (`daniella-santana-portrait.webp`) continua no disco, só
 * deixa de ser referenciada em `src/config/images.ts`, caso o usuário queira
 * usá-la de novo depois.
 */
...
<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}" viewBox="0 0 ${width} ${height}">
  ...
  <text x="${cx}" y="${cy + iconSize * 1.35 + Math.max(18, width * 0.038)}" font-family="Arial, sans-serif" font-size="${Math.max(11, width * 0.022)}" fill="#747f8b" text-anchor="middle">Daniella Santana</text>
</svg>`;

const fullPath = path.join(ROOT, "team", "daniella-santana-portrait-placeholder.webp");
await mkdir(path.dirname(fullPath), { recursive: true });
await sharp(Buffer.from(svg)).webp({ quality: 80 }).toFile(fullPath);
console.log("OK team/daniella-santana-portrait-placeholder.webp");
